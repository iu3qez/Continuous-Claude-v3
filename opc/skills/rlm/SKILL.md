...
1. Initialise the REPL state
   ```bash
   python "$HOME/.claude/skills/rlm/scripts/rlm_repl.py" --state "$HOME/.claude/rlm_state/state.pkl" init <context_path>
   python "$HOME/.claude/skills/rlm/scripts/rlm_repl.py" --state "$HOME/.claude/rlm_state/state.pkl" status
   ```

2. Scout the context quickly
   ```bash
   python "$HOME/.claude/skills/rlm/scripts/rlm_repl.py" --state "$HOME/.claude/rlm_state/state.pkl" exec -c "print(peek(0, 3000))"
   python "$HOME/.claude/skills/rlm/scripts/rlm_repl.py" --state "$HOME/.claude/rlm_state/state.pkl" exec -c "print(peek(len(content)-3000, len(content)))"
   ```
...
4. Materialise chunks as files (so subagents can read them)
   ```bash
   python "$HOME/.claude/skills/rlm/scripts/rlm_repl.py" --state "$HOME/.claude/rlm_state/state.pkl" exec <<'PY'
   import os
   paths = write_chunks(os.path.expanduser('~/.claude/rlm_state/chunks'), size=200000, overlap=0)
   print(len(paths))
   print(paths[:5])
   PY
   ```
...
After completing an RLM task, clean up state:
```bash
python "$HOME/.claude/skills/rlm/scripts/rlm_repl.py" --state "$HOME/.claude/rlm_state/state.pkl" reset
rm -rf "$HOME/.claude/rlm_state/chunks/"
```
